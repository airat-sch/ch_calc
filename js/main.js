...
function calculate() {
	...
	let c_ag_delivery = parseFloat(document.getElementById("c_ag_delivery").value.replace(/,/, '.')); //Доставка до Агента
	let c_log_price = parseFloat(document.getElementById("c_log_price").value.replace(/,/, '.')); //Стоимость логистики
	...
	let c_pokat = parseFloat(document.getElementById("c_pokat").value.replace(/,/, '.')); //Покатушки (исх. Из выкупа 50%)
